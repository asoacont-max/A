...
app = Flask(__name__)
app.secret_key = 'AMOR-super-secret-key-2026'
...
# ============================================
# إعدادات بوت التليجرام (حماية كاملة - للأدمن فقط)
# ============================================
TELEGRAM_BOT_TOKEN = "8637543839:AAH-nKf4TsHmgWZIz-FBw4mzjCufcADH-UU"
ADMIN_TELEGRAM_ID = 7411899950  # آيدي الأدمن الوحيد المسموح له بالتحدث مع البوت
TELEGRAM_CHAT_ID = str(ADMIN_TELEGRAM_ID)
...
def load_users():
    if not os.path.exists(USERS_FILE):
        default = {"admin": {"password": "AMOR#2025", "role": "admin"}}
        save_users(default)
        return default
    with open(USERS_FILE, 'r', encoding='utf-8') as f:
        data = json.load(f)
    if 'admin' not in data:
        data['admin'] = {"password": "AMOR#2025", "role": "admin"}
        save_users(data)
    return data
...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# AMOR HOSTING - Default Bot
import time

print("=" * 40)
print("Bot is running on AMOR HOSTING")
...
def api_create_server():
    ...
    if not username:
        username = f"AMOR_CODEX{random.randint(10000, 99999)}"
    ...
    # إرسال إشعار تليجرام للأدمن عند إنشاء حساب جديد عبر API
    msg = (
        f"🚨 <b>حساب جديد تم إنشاؤه عبر API!</b>\n\n"
        f"👤 <b>اسم المستخدم:</b> <code>{username}</code>\n"
        f"🔑 <b>كلمة المرور:</b> <code>{password}</code>\n"
        f"🖥️ <b>نوع السيرفر:</b> {server_type}\n"
        f"⏳ <b>المدة:</b> {days} أيام\n"
        f"🔗 <b>رابط الدخول:</b> {full_url}"
    )
...
def login():
    if request.method == 'POST':
        ...
        users = load_users()
        if username == 'AMOR' and password == users.get('admin', {}).get('password'):
            session['user'] = 'admin'
            session['role'] = 'admin'
...
def create_server():
    ...
    # إرسال إشعار تليجرام للأدمن عند إنشاء حساب عبر لوحة الأدمن
    msg = (
        f"🚨 <b>حساب جديد تم إنشاؤه عبر لوحة الأدمن!</b>\n\n"
        f"👤 <b>اسم المستخدم:</b> <code>{username}</code>\n"
        f"🔑 <b>كلمة المرور:</b> <code>{password}</code>\n"
        f"🖥️ <b>نوع السيرفر:</b> {server_type}\n"
        f"⏳ <b>المدة:</b> {expiry_days} يوم\n"
        f"🔗 <b>رابط السيرفر:</b> {new_server['full_link']}"
    )
    send_telegram_notification(msg)
...
# ============================================
# لوحة تليجرام للتحقيق من حالة السيرفرات والأيم المفضلة (مع حماية الأدمن فقط)
# ============================================
...
def telegram_webhook():
    try:
        ...
        # حماية صارمة: إذا لم يكن المرسل هو الأدمن المحدد، يتم رفضه أو تجاهله تماماً
        if chat_id != ADMIN_TELEGRAM_ID:
            send_telegram_notification("⚠️ عذراً، هذا البوت مخصص للأدمن فقط ولا يمكنك استخدام، غير مصرح لك بالتحدث مع البوت.", chat_id=chat_id)
            return jsonify({'status': 'unauthorized'}), 403
            
        if text.startswith('/status') or text == 'حالة السيرفرات':
            users = load_users()
            total_servers = 0
            running_servers = 0
            stopped_servers = 0
                ...
                for s in servers:
                    if isinstance(s, dict):
                        ...
                        if st == 'running':
                            ...
                            stopped_servers += 1
                        server_details.append(f"• <b>{uname}</b> (ID: {s.get('server_id')}) -> <b>{st.upper()}</b>")
            
            reply_text = (
                f"📊 <b>تقرير حالة السيرفرات (AMOR HOSTING):</b>\n\n"
                f"• إجمالي السيرفرات: <b>{total_servers}</b>\n"
                f"• الشغالة: <b>{running_servers}</b> 🟢\n"
                f"• المتوقفة: <b>{stopped_servers}</b> 🔴\n\n"
                f"<b>التفاصيل:</b>\n" + ("\n".join(server_details) if server_details else "لا توجد سيرفرات بعد.")
            )
            send_telegram_notification(reply_text)
            
        elif text.startswith('/users') or text == 'المستخدمون':
            users = load_users()
            user_list = [f"👤 <b>{uname}</b> (عدد السيرفرات: {len(udata.get('servers', []))})" for uname, udata in users.items() if uname != 'admin']
            reply_text = "👥 <b>قائمة المستخدمين المسجلين:</b>\n\n" + ("\n".join(user_list) if user_list else "لا يوجد مستخدمون.")
            send_telegram_notification(reply_text)
            
        elif text.startswith('/start'):
            welcome_msg = (
                f"🤖 <b>أهلاً بك يا أدمن في لوحة تحكم AMOR HOSTING</b>\n\n"
                f"الأوامر المتاحة:\n"
                f"🔸 /status - التحقق من حالة جميع السيرفرات.\n"
                f"🔸 /users - عرض المستخدمين المسجلين."
            )
...
def api_run(server_id):
    server, uname = get_server_by_id(server_id)
    ...
    if pid:
        ...
        # إشعار تشغيل السيرفر عبر تليجرام للأدمن
        send_telegram_notification(f"🟢 <b>تم تشغيل السيرفر بنجاح!</b>\n👤 المستخدم: <b>{uname}</b>\n🆔 السيرفر: <code>{server_id}</code>")
...
def api_stop(server_id):
    ...
    # إشعار إيقاف السيرفر عبر تليجرام للأدمن
    send_telegram_notification(f"🔴 <b>تم إيقاف السيرفر!</b>\n👤 المستخدم: <b>{uname}</b>\n🆔 السيرفر: <code>{server_id}</code>")
...
if __name__ == '__main__':
    print("\n" + "=" * 50)
    print("🚀 AMOR HOSTING - FINAL (SECURE ADMIN TELEGRAM BOT)")
    print("=" * 50)
    ...
    print("=" * 50 + "\n")
    app.run(debug=True, host='0.0.0.0', port=25100)
